...
class File
{
	...
	function saveUploadedFile($uploaded_file, $folder_path)
	{
		...
		if($saved)
		{
			...
			$file_name = $this->getFilename($file_item[$this->dbId], $file_item[$this->dbName]);
			$orig_file_path = $folder_path . $file_name;

			resizePic($this->files_prefix, $orig_file_path, $folder_path, '640', 0, 0, $file_name);
	...
	function createThumbFromFileOriginal($file_id, $folder_path, $width)
	{
		$file = $this->get($file_id);
		...
		resizePic($this->files_prefix, $orig_file_path, $folder_path, $width, 0, 0, $thumb_file_name);
